...
export function Pricing() {
  ...
  return <section className="py-24 bg-card/50 relative overflow-hidden">
      {/* Background gradient */}
      ...
      <div className="container relative z-10 mx-auto px-4">
        ...
        <div className="flex justify-center mb-12">
          <div className="inline-flex items-center gap-1 p-1 rounded-full bg-muted/50 border border-border/50">
            ...
            return <button key={role} onClick={() => setSelectedRole(role)} className={cn("flex items-center gap-2 px-4 py-2 rounded-full text-sm font-medium transition-all duration-300", isActive ? "bg-primary text-primary-foreground shadow-md" : "text-muted-foreground hover:text-foreground hover:bg-muted")}>
        ...
        <div className="text-center mt-12">
          {selectedRole === "student" && <p className="text-muted-foreground">
...
function PricingCard({
  ...
  return <div className={cn("relative p-6 rounded-2xl border transition-all duration-300 hover:shadow-xl", popular ? "bg-gradient-to-b from-primary/10 to-transparent border-primary/50 shadow-glow" : "bg-card border-border/50 hover:border-primary/30")}>
      {popular && <div className="absolute -top-3 left-1/2 -translate-x-1/2">
      ...
      <div className="mb-6">
        <div className="flex items-baseline gap-1">
          {price === 0 ? <span className="text-4xl font-bold text-foreground">$0</span> : <>
      ...
      <ul className="space-y-3 mb-8">
        {features.map(feature => <li key={feature} className="flex items-start gap-2">
            <Check className="w-5 h-5 text-success flex-shrink-0 mt-0.5" />
            <span className="text-sm text-foreground">{feature}</span>
          </li>)}
      </ul>

      <Button variant="hero" className={cn("w-full", !popular && "opacity-80 hover:opacity-100")} asChild>
